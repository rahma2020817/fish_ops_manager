...
function CreatedrecetteElement({ data, setData }) {
  const navigate = useNavigate();
  const handlerecette = (id) => {
    navigate(`/recetteDetails/${id}`);
  };
  // console.log(data);
  return (
    <div className="flex flex-wrap gap-6">
      {data?.map((e, index) => {
        return (
          <div
            key={index}
            onClick={(event) => handlerecette(e._id)}
            title="recette"
          ...
          >
            {/* <Link to={"/recetteDetails"}> */}
            <div className=" w-full p-2 flex justify-between items-center ">
              <h2 className="heading3 inline font-medium">{e.recettePosition}</h2>
              <button
            ...
            <div className="w-full flex h-28">
              <div className="w-1/2 flex flex-col justify-center text-center">
                <div className="heading4">Totall Recettes</div>
                <div className="heading4 font-medium">{e.applicants_no}</div>
              </div>

              <div className="w-1/2  flex flex-col justify-center text-center">
                <div className="heading4  ">Recettes</div>
                <div className="heading4 font-medium">0</div>
              </div>
            </div>

            {/* PART TO SHOW SHARE recette-ID AND SHARE BUTTONS */}

            <div className="flex flex-row w-full pb-4">
              <div className=" w-4/5 ml-4">
                <p className="text-sm heading4">recette-ID: {index}</p>
              </div>
...
export default CreatedrecetteElement;
